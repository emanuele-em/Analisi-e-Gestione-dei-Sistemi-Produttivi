...
\newglossaryentry{tempoInterarrivo}{
    name={\ensuremath{\bm{\frac{1}{\lambda} = T_a}}},
...
\newglossaryentry{kendall}{
    name={\ensuremath{\bm{M/M/1/nMax}}},
    description={è la notazione di Kendall, i parametri hanno il seguente significato:
        \begin{itemize}
...
\newglossaryentry{equilibrioSistema}{
    name={Sistema in equilibrio},
    description={Il sistema è in equilibrio quando preso un nodo, il numero in ingressi è uguale al numero di uscite quindi quando \(IN=OUT\)}
}

\newglossaryentry{Stati}{
    name={Stati},
    description={Gli stati di un sistema si studiano con dei grafi in cui i nodi rappresentano gli stati del sistema con le informazioni in aggregato:
    \begin{conditions*}
...
\newglossaryentry{balkingRate}{
    name={Balking Rate},
    description={\(\lambda p_{nMax}\) rappresenta tutti i job che cercano di entrare quando il sistema limitato è saturo, di conseguenza vengono scartati. Rappresenta la domanda persa}
}

\newglossaryentry{probabilitaSistemaNonSaturo}{
    name={\ensuremath{\bm{1-p_{nMax}}}},
    description={probabilità che il sistema non sia saturo}
}

\newglossaryentry{idletime}{
    name={Idle Time},
    description={Tempo in cui il sistema è completamente vuoto o fermo}
}

\newglossaryentry{busytime}{
    name={busy time},
    description={tempo in cui il sistema lavora, in un sistema illimitato corrisponde all'utilizzo \(u\), in un sistema limitato invece, dato che l'utilizzo non ha significato se \(>1\) non è possibile intercambiare i due significati}
}

\newglossaryentry{lambdaeffettivo}{
    name={\ensuremath{\bm{\lambda_e}}},
    description={\(\lambda\) effettivo, considerando il buffer e la macchina separati, se il buffer è limitato allora \(\lambda_e<\lambda\)}
}

\newglossaryentry{lambdaeffettivomm1}{
    name={\ensuremath{\bm{\lambda_e^{M/M/1}}}},
    description={\(\lambda(1-p_{nMax})\)}
}